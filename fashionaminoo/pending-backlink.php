...
{
  $Backlink_Approved_id = $_GET['approved_id'];

    ////Upadte query
    $sql = "UPDATE webinformer_backlink SET 
        backlink_status='Approved'
        WHERE id='$Backlink_Approved_id' ";

        if ($conn->query($sql) === TRUE) 
        {
          header('location:pending-backlink.php');
        }
}
?>
...
<div style="width: 95%; margin-bottom: 80px; margin-top: 8px;">
    ...
    <h1 style="font-weight: bold; color: #FF8300; font-size: 45px;">All Pending Backlink 
      <span style="color:black; margin-left:5px; font-size:16px;">(<?php echo $total_Pending_backlink; ?>)</span>
    </h1>
...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
  ...
  $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM `webinformer_backlink` WHERE backlink_status='Pending' AND active_status='' ");
  ...
  ////////////////////////////[Pagination Code End]/////////////////////////////////////
  		...
  		$no=0;
        $query1 = "SELECT  * FROM  webinformer_backlink WHERE backlink_status='Pending' AND active_status='' order by 1 DESC  LIMIT $offset, $total_records_per_page ";
        $query2 = mysqli_query($conn, $query1);
    ...
    <tr>
      ...
      <td style="color: green; font-weight: bold;">
        <a href="?approved_id=<?php echo $backlnk_id; ?>">
