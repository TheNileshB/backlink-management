...
if (isset($_GET['my_url_data']))
{
  $my_url_data = $_GET['my_url_data'];
...
////////GET Virable 2 ////////////
$backlink_type = "";
if (isset($_GET['backlink_type']))
{
   $backlink_type = $_GET['backlink_type'];
}
...
<div style="background: #E7E7E7; width: 100%; height: auto; overflow: hidden;">
  <div style="width: 100%; height: auto; background: ; padding-top: 10px;">
    <center>
      <h1 style="font-weight: bold;">My URL Data</h1>
    </center>
    <div class="row" style="margin-bottom: 40px; margin-top: 20px;">
      <div class="col-xl-3 col-lg-3" style="background: ; margin: 0; padding:;">
        <form action="" method="post">
        <select class="browser-default custom-select selectpicker" name="url_selection" style="float: right; width: 210px; height: 43px;">
          <option value="Backlink URL">Backlink URL</option>
          <option selected="selected" value="My URL">My URL</option>
      ...
      <div class="col-xl-8 col-lg-8" style="text-align: left; background: ; margin: 0; padding:;">
        <div class="row">
          ...
          <input style="height: 43px; padding: 10px; border: none; width: 100%;" type="search" name="url_search" placeholder="Enter Anchor Keyword" name="backlink_url_search" value="<?php echo $my_url_data; ?>">
        </div>
...
<center>
  ...
  <div>
    <a style="margin-right: 10px;" href="my-url-data.php?my_url_data=<?php echo $my_url_data; ?>">
      <button class="btn btn-info">
        All
      </button>
    </a>

    <a style="margin-right: 10px;" href="my-url-data.php?my_url_data=<?php echo $my_url_data; ?>&backlink_type=DoFollow">
      <button class="btn btn-info">
        DoFollow
      </button>
    </a>

    <a href="my-url-data.php?my_url_data=<?php echo $my_url_data; ?>&backlink_type=NoFollow">
      <button class="btn btn-info">
        NoFollow
      </button>
    </a>
...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
  ...
  ////////////////////////////[Pagination Code End]/////////////////////////////////////
      ...
      if ($my_url_data=="https://fashionaminoo.com/") 
      {
        if ($backlink_type=="DoFollow") 
        {
            ...
            $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records 
                            ...
                            WHERE my_link='$my_url_data' AND active_status=''
                            And link_type='DoFollow'  order by 1 DESC ");
            $total_records = mysqli_fetch_array($result_count);
            $total_records = $total_records['total_records'];
            $total_no_of_pages = ceil($total_records / $total_records_per_page);
            $second_last = $total_no_of_pages - 1; // total page minus 1

          $query1 = "SELECT  * FROM  webinformer_backlink WHERE my_link='$my_url_data' And link_type='DoFollow' AND active_status=''  order by 1 DESC  LIMIT $offset, $total_records_per_page";
        }

        if ($backlink_type=="NoFollow") 
        {
            ...
            $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records 
                            FROM `webinformer_backlink` 
                            WHERE my_link='$my_url_data' AND active_status=''
                            And link_type='NoFollow'  order by 1 DESC ");
            $total_records = mysqli_fetch_array($result_count);
            $total_records = $total_records['total_records'];
            $total_no_of_pages = ceil($total_records / $total_records_per_page);
            $second_last = $total_no_of_pages - 1; // total page minus 1

          $query1 = "SELECT  * FROM  webinformer_backlink WHERE my_link='$my_url_data' And link_type='NoFollow' AND active_status=''  order by 1 DESC  LIMIT $offset, $total_records_per_page ";
        }
        ...
        if ($backlink_type=="") 
        {
            ...
            $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records 
                            FROM `webinformer_backlink` 
                            WHERE my_link='$my_url_data' AND active_status='' order by 1 DESC ");
            $total_records = mysqli_fetch_array($result_count);
            $total_records = $total_records['total_records'];
            $total_no_of_pages = ceil($total_records / $total_records_per_page);
            $second_last = $total_no_of_pages - 1; // total page minus 1
          ...
          $query1 = "SELECT  * FROM  webinformer_backlink WHERE my_link='$my_url_data' AND active_status=''  order by 1 DESC  LIMIT $offset, $total_records_per_page ";
        }
      }
      else
      {
        if ($backlink_type=="DoFollow") 
        {
          ...
          $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records 
                            FROM `webinformer_backlink` 
                            WHERE my_link Like '%$my_url_data%' AND active_status=''
                            And link_type='DoFollow'  order by 1 DESC ");
            $total_records = mysqli_fetch_array($result_count);
            $total_records = $total_records['total_records'];
            $total_no_of_pages = ceil($total_records / $total_records_per_page);
          ...
          $query1 = "SELECT  * FROM  webinformer_backlink WHERE my_link Like '%$my_url_data%' AND active_status='' And link_type='DoFollow'  order by 1 DESC  LIMIT $offset, $total_records_per_page";
        }
        ...
        if ($backlink_type=="NoFollow") 
        {
          ...
          $query1 = "SELECT  * FROM  webinformer_backlink WHERE my_link Like '%$my_url_data%' And link_type='NoFollow' AND active_status='' order by 1 DESC  LIMIT $offset, $total_records_per_page";
        }
        ...
        if ($backlink_type=="") 
        {
            ...
            $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records 
                            FROM `webinformer_backlink` 
                            WHERE my_link Like '%$my_url_data%' AND active_status='' order by 1 DESC ");
            $total_records = mysqli_fetch_array($result_count);
            $total_records = $total_records['total_records'];
            $total_no_of_pages = ceil($total_records / $total_records_per_page);
            $second_last = $total_no_of_pages - 1; // total page minus 1
          ...
          $query1 = "SELECT  * FROM  webinformer_backlink WHERE my_link Like '%$my_url_data%' AND active_status=''  order by 1 DESC  LIMIT $offset, $total_records_per_page ";
        }
      }
...
<ul class="pagination_11">
  ...
  <li <?php if($page_no <= 1){ echo "class='disabled'"; } ?>>
    <a <?php if($page_no > 1){ echo "href='?my_url_data=$my_url_data&backlink_type=$backlink_type&page_no=$previous_page'"; } ?>>Previous</a>
  </li>
    ...
    for ($counter = 1; $counter <= $total_no_of_pages; $counter++){
      if ($counter == $page_no) {
       echo "<li class='active'><a>$counter</a></li>";  
           ...
           echo "<li><a href='?my_url_data=$my_url_data&backlink_type=$backlink_type&page_no=$counter'>$counter</a></li>";
        }
        }
  }
  ...
  if($page_no <= 4) {     
   for ($counter = 1; $counter < 8; $counter++){     
    ...
    echo "<li><a>...</a></li>";
    echo "<li><a href='?my_url_data=$my_url_data&backlink_type=$backlink_type&page_no=$second_last'>$second_last</a></li>";
    echo "<li><a href='?my_url_data=$my_url_data&backlink_type=$backlink_type&page_no=$total_no_of_pages'>$total_no_of_pages</a></li>";
    }

   elseif($page_no > 4 && $page_no < $total_no_of_pages - 4) {     
    echo "<li><a href='?my_url_data=$my_url_data&backlink_type=$backlink_type&page_no=1'>1</a></li>";
    echo "<li><a href='?my_url_data=$my_url_data&backlink_type=$backlink_type&page_no=2'>2</a></li>";
        echo "<li><a>...</a></li>";
...
?>
  ...
  <a <?php if($page_no < $total_no_of_pages) { echo "href='?my_url_data=$my_url_data&backlink_type=$backlink_type&page_no=$next_page'"; } ?>>Next</a>
  </li>
    <?php if($page_no < $total_no_of_pages){
    echo "<li><a href='?my_url_data=$my_url_data&backlink_type=$backlink_type&page_no=$total_no_of_pages'>Last &rsaquo;&rsaquo;</a></li>";
    } ?>
</ul>
<!-----------------------------------[Pagination Code Start]----------------------------------------------->
