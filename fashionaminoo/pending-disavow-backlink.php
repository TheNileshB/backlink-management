...
{
  $disavow_approved_id = $_GET['approved_id'];

    ////Upadte query
    $sql = "UPDATE disavow_backlink SET 
        status='Approved'
        WHERE id='$disavow_approved_id' ";

        if ($conn->query($sql) === TRUE) 
        {
          header('location:pending-disavow-backlink.php');
        }
}
?>
...
<div style="width: 95%; margin-bottom: 80px; margin-top: 8px;">
    ...
    <h1 style="font-weight: bold; color: green; font-size: 45px;">Pending Disavow
      <span style="color:black; margin-left:5px; font-size:16px;">(<?php echo $total_Pending_Disavow_backlink; ?>)</span>
    </h1>
    <br>
...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
  ...
  $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM `disavow_backlink` WHERE status='Pending'");
  ...
  ////////////////////////////[Pagination Code End]/////////////////////////////////////
  		...
  		$no=0;
        $query1 = "SELECT  * FROM  disavow_backlink WHERE status='Pending' order by 1 DESC LIMIT $offset, $total_records_per_page ";
        $query2 = mysqli_query($conn, $query1);
    ...
    <tr style="text-align: center;">
      ...
      <td style="color: green; font-weight: bold;">
        <a href="?approved_id=<?php echo $id; ?>">
